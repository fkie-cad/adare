...
def find_ovmf_firmware() -> Tuple[str, str]:
    ...
    Example:
        >>> code, vars_template = find_ovmf_firmware()
        >>> print(code)
        '/usr/share/OVMF/OVMF_CODE.fd'
    """
    searched_paths = []

    for code_path, vars_path in OVMF_SEARCH_PATHS:
...
def create_nvram_for_vm(vm_name: str, vm_config_dir: Path) -> str:
    ...
    Example:
        >>> nvram = create_nvram_for_vm("windows-10", Path("~/.adare/qemu/vms"))
    """
    _, vars_template = find_ovmf_firmware()
