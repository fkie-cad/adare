    ...
    async def _execute_test(self, action: ActionTestAction, parent_event_id: str = None, event_emitter = None) -> ActionResult:
        ...
        try:
            ...
            if test_class and getattr(test_class, 'execute_on_host', False):
                # HOST-SIDE EXECUTION
                log.debug(f"Action Executor: Routing test '{action.name}' to host executor")

                # Create host test executor if not exists (lazy init)
                if not hasattr(self, 'host_test_executor'):
                    ...
                    self.host_test_executor = HostTestExecutor(
                        ...
                        mcp_client=self.target_resolver.mcp_client,
                        playbook_dir=playbook_dir,
                        experiment_dir=self.experiment_run_directory
                    )
                    log.debug("Action Executor: Created HostTestExecutor")
                ...
                log.debug(f"Action Executor: Routing test '{action.name}' to VM executor")
