...
The core functionality of this framework lies in running experiments and facilitating both the viewing and publishing of their results.
An experiment consists of a series of :term:`actions<action>` and :term:`tests<test>`, all executed on a virtual machine.
The :term:`actions<action>`, which are grouped into an :term:`action set`, aim to simulate user interactions with the virtual machine, such as deleting a file by clicking and pressing delete.
Conversely, a :term:`test set` which comprises multiple :term:`tests<test>`, is used to verify the state of forensic artifacts as well as the state of the machine by comparing them against a known ground truth.
...
:term:`Actions<action>` are specified in a Python file, named the :term:`action set file`, located within the experiment directory.
A :term:`test set<test set>` is defined in a YAML file, termed the :term:`test set file`, also within the experiment directory.
...
.. glossary::
   ...
   scenario
      A scenario encapsulates a set of experiments that aim to investigate the same forensic artifact.
      It is not represented by a file or a table in the database.
      Experiments can be assigned to multiple scenarios.
      However, whether an experiment is part of a scenario plays no role in the execution of the experiment.

   action
      An action is a single simulated interaction between a user and the virtual machine.
      This can be, for example, clicking on a file or pressing a key.

   action set
      An action set is a series of actions arranged in an execution flow with possibly branches.

   action set file
      The action set file is unique per experiment python file including the execution flow of actions.
   ...
   test set file
      The test set file is a YAML file including a test set.
...
::
    ...
    │   │   ├── test_experiment.py      # action set file
    │   │   ├── test_experiment.yml     # test set file
    │   │   ├── metadata.yml            # metadata file
    └── └── └── img                     # directory for the images used within the action set file
