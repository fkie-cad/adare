...
1. gui scenario (sequence of gui actions such as mouse clicks and key presses)
2. parsing artifacts
3. analyze the parsed artifacts


The user provides a so called *scenario*, consisting of a *gui automation file* and a *parse and test file*.
Inside the *gui automation file* is specified


The experiments itself consists each of a *gui automation* and *parse & test* part.
During the *gui automation* part an user written gui scenario, such as clicking on a file and deleting it to the Trash Bin, will be executed.
