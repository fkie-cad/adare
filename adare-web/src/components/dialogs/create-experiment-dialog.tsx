      ...
      <DialogContent>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <DialogHeader>
            ...
            <DialogDescription>Create a new experiment in a project.</DialogDescription>
          </DialogHeader>

          <FormField label="Project" htmlFor="exp-project" required>
            <select
              id="exp-project"
              value={projectPath}
          ...
          <FormField label="Name" htmlFor="exp-name" required>
            <Input
              id="exp-name"
              value={name}
          ...
          {mutation.isError && (
            <div className="rounded-md border border-destructive bg-destructive/10 p-2 text-xs text-destructive">
              {(mutation.error as Error)?.message ?? 'Failed to create experiment.'}
            </div>
          )}
