...
class TestSnapshotMixin:
    ...
    @patch('subprocess.run')
    def test_create_external_snapshot_with_quiesce_flag(self, mock_run, mixin):
        ...
        calls = mock_run.call_args_list
        # Verify --quiesce flag is present in snapshot command
        snap_calls = [c for c in calls if 'snapshot-create-as' in str(c[0][0])]
        assert len(snap_calls) == 1
        assert '--quiesce' in snap_calls[0][0][0]
